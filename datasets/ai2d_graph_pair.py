...
class AI2DGraphPair(GraphPairDataset):
    ...
    def __init__(self, dirPath=None, split=None, config=None, images=None, test=False):
        ...
        if images is not None:
		    ...
		    rescale=1.0
                    ...
                    self.images.append({'id':image, 'imagePath':imagePath, 'annotationPath':jsonPath, 'rescaled':rescale, 'imageName':imageName[:imageName.rfind('.')]})
    ...
    def parseAnn(self,annotations,scale):
        ...
        bbs = torch.tensor(bbs)[None,:,:] #add batch dim on front
        return bbs,ids,3


    def transformBB(item,scale,classNum):
        if classNum==2:
            rect = item['rectangle']
