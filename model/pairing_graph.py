    ...
    def forward(self, image, gtBBs=None, otherThresh=None, otherThreshIntur=None, hard_detect_limit=300):
        ...
        if useBBs.size(0)>0:
            ...
            if len(candidates)==0:
                ...
                stackedEdgeFeatWindows = self.roi_align(final_features,rois.to(final_features.device))
                final_features=None
                ...
                #adjacencyMatrix = torch.sparse.FloatTensor(candidateLocs,ones,torch.Size([useBBs.size(0),useBBs.size(0)]))

                #assert(edgeFeats.requries_grad)
                #edge_features = torch.sparse.FloatTensor(candidateLocs,edgeFeats,torch.Size([useBBs.size(0),useBBs.size(0),edgeFeats.size(1)]))
